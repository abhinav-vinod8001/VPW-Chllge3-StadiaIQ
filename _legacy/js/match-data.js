/* ============================================
   StadiumAI — FIFA World Cup 2026 Match Data
   Real venues, real locations, real schedule
   ============================================ */

const MatchData = (() => {

  // All 16 FIFA World Cup 2026 USA venues with real coordinates
  const venues = {
    'metlife': {
  ...
  // FIFA World Cup 2026 schedule (knockout stage — user's current date is July 12)
  const matches = [
    // Group Stage (already completed by July 12)
    // Round of 16 (most completed)
  ...
  // Traffic conditions around venues (simulated)
  ...
  function getUSAVenues() {
    return Object.values(venues).filter(v =>
      !v.city.includes('Mexico') && !v.city.includes('Canada')
    );
  }
  ...
  function getCurrentMatch() {
    return getLiveMatch() || getUpcomingMatches()[0] || null;
  }
  ...
  function getCompletedMatches() {
    return matches.filter(m => m.status === 'completed').sort((a, b) => new Date(b.date) - new Date(a.date));
  }

  function getMatchesByVenue(venueId) {
  ...
  function searchMatches(query) {
    ...
    return matches.filter(m => {
      ...
      return m.teamA.toLowerCase().includes(q) ||
             m.teamB.toLowerCase().includes(q) ||
             m.round.toLowerCase().includes(q) ||
             (venue && venue.name.toLowerCase().includes(q)) ||
             (venue && venue.city.toLowerCase().includes(q));
    });
  }

  function getTraffic(venueId) {
    return trafficConditions[venueId] || trafficConditions.metlife;
  }

  function getGoogleMapsUrl(venueId) {
    const venue = venues[venueId];
    if (!venue) return '#';
    return `https://www.google.com/maps/dir/?api=1&destination=${venue.lat},${venue.lng}&travelmode=driving`;
  }

  function getGoogleMapsTransitUrl(venueId) {
    const venue = venues[venueId];
    if (!venue) return '#';
  ...
  function getMatchStatus(match) {
    if (match.status === 'live') return { label: 'LIVE', color: 'danger', dot: true };
    if (match.status === 'completed') return { label: `${match.scoreA} - ${match.scoreB}`, color: 'success', dot: false };
    return { label: match.time + ' ET', color: 'info', dot: false };
  }
