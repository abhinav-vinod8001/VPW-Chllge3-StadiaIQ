...
describe('aiEngine', () => {
  ...
  it('should return mock fallback if fetch fails for AI response', async () => {
    ...
    expect(response.text.includes('Could not reach live telemetry API')).toBe(true);
  });
  ...
  it('should return parsed response from Groq API', async () => {
    const mockResponse = {
      ok: true,
      json: async () => ({
        choices: [{ message: { content: 'Gate 4 is located at the north end.' } }]
      })
    };
  ...
  it('should return fallback if PA broadcast fails', async () => {
    ...
    const response = await generatePABroadcastAsync({ type: 'Gate 4', location: 'Gate 4' }, 'en', 'metlife');
    expect(response).toBeDefined();
  ...
  it('should return generated PA broadcast from Groq API', async () => {
    const mockResponse = {
      ok: true,
      json: async () => ({
        choices: [{ message: { content: 'Attention fans, please proceed to Gate 4.' } }]
      })
    };
    global.fetch = vi.fn().mockResolvedValue(mockResponse);
