...
export function detectLanguage(query) {
  ...
  if (/[áéíóúñ¿¡]/.test(q) || ['hola', 'puerta', 'asiento', 'donde', 'gracias', 'qué', 'como'].some(w => q.includes(w))) return 'es';
  if (/[éèêàùç]/.test(q) || ['bonjour', 'porte', 'siège', 'ou', 'merci', 'comment'].some(w => q.includes(w))) return 'fr';
  if (/[ãõâêí]/.test(q) || ['olá', 'portão', 'assento', 'onde', 'obrigado', 'como'].some(w => q.includes(w))) return 'pt';
  return savedLang || 'en';
}
...
export function generateAIResponse(query) {
  ...
  if (q.includes('section') || q.includes('sec') || q.includes('seat') || q.includes('gate') || q.includes('where') || q.includes('restroom') || q.includes('food')) {
    return venueResponses.navigation[lang] || venueResponses.navigation.en;
  }
  if (q.includes('crowd') || q.includes('busy') || q.includes('wait') || q.includes('queue') || q.includes('line')) {
    return venueResponses.crowd[lang] || venueResponses.crowd.en;
  }
  if (q.includes('train') || q.includes('metro') || q.includes('bus') || q.includes('drive') || q.includes('uber') || q.includes('home') || q.includes('route') || q.includes('traffic')) {
    return venueResponses.transport[lang] || venueResponses.transport.en;
  }
  if (q.includes('eco') || q.includes('carbon') || q.includes('recycle') || q.includes('sustain')) {
    return venueResponses.sustainability[lang] || venueResponses.sustainability.en;
  }
...
export async function generateAIResponseAsync(query, venueId = 'metlife', chatHistory = []) {
  ...
  try {
    ...
    const lang = detectLanguage(query);
    const langNames = { en: 'English', es: 'Spanish (Español)', fr: 'French (Français)', pt: 'Portuguese (Português)' };
...
6. FIRMLY REJECT any queries related to personal life problems, math, coding, or any topics entirely unrelated to football, the World Cup, or stadium operations. Politely explain that as StadiaIQ, you only assist with the World Cup experience.`;
    ...
    const reply = data.choices?.[0]?.message?.content || generateAIResponse(query);
    ...
    console.warn('Groq live inference error, falling back to local engine:', error);
    const fallbackText = generateAIResponse(query);
    return {
      text: `${fallbackText}\n\n---\n*⚠️ **System Notice:** Could not reach live telemetry API (` + error.message + `). Showing high-accuracy local rules.*`,
      isGroq: false,
...
Direct fans clearly on how to proceed safely and smoothly (e.g., recommend express Gate 4 or step-free routes). Keep the script exact, professional, and under 60 words so it can be broadcast clearly over concourse speakers and LED ribbon displays right now. Prefix with 📢 **PA BROADCAST SCRIPT (${targetLang.toUpperCase()})**:`;
    ...
    const data = await response.json();
    return {
      script: data.choices?.[0]?.message?.content || `📢 PA Broadcast: Attention fans at ${venue.name}, please follow steward guidance near ${incidentOrBottleneck.location || 'Gate 2'}.`,
      isGroq: true,
