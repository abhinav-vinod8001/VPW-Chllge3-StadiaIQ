...
let globalTelemetryState = {
  ...
  liveAlerts: [
    { id: 'ALT-1', road: 'NJ Route 3 (Eastbound)', status: 'heavy', delay: 25, color: 'danger', time: '19:42 ET' },
    { id: 'ALT-2', road: 'I-95 / NJ Turnpike Exit 16W', status: 'moderate', delay: 14, color: 'warning', time: '19:48 ET' },
  ],
  incidents: [
    { id: 'INC-109', time: '19:42 ET', type: 'Medical Assist Required', location: 'Section 114 (Concourse Level 1)', status: 'Dispatched', priority: 'High', color: 'danger' },
    { id: 'INC-110', time: '19:48 ET', type: 'Turnstile Scanner Sync Delay', location: 'Gate 2 (East Concourse Bay 4)', status: 'Resolving', priority: 'Medium', color: 'warning' },
    { id: 'INC-111', time: '19:55 ET', type: 'Concession Stock Replenishment', location: 'Food Court North (Stand 12)', status: 'Restocked', priority: 'Low', color: 'success' },
  ],
  lastTick: Date.now()
};
...
if (typeof window !== 'undefined') {
  setInterval(() => {
    ...
    const newMetro = globalTelemetryState.transitCountdowns.metroGreen <= 0 ? 240 : globalTelemetryState.transitCountdowns.metroGreen - 3;
    const newShuttle = globalTelemetryState.transitCountdowns.fanShuttle <= 0 ? 600 : globalTelemetryState.transitCountdowns.fanShuttle - 3;
    const newCommuter = globalTelemetryState.transitCountdowns.commuterRail <= 0 ? 1200 : globalTelemetryState.transitCountdowns.commuterRail - 3;

    // 2. Micro-fluctuations in attendance (real-time turnstile entries)
    const delta = Math.floor(Math.random() * 7) - 1; // -1 to +5 fans
    const newAttendance = Math.min(82500, Math.max(60000, globalTelemetryState.attendance + delta));
...
export function resolveIncidentGlobal(incidentId) {
  globalTelemetryState = {
    ...
    incidents: globalTelemetryState.incidents.map(inc => 
      inc.id === incidentId ? { ...inc, status: 'Resolved', color: 'success' } : inc
    )
  };
  notifyListeners();
}
