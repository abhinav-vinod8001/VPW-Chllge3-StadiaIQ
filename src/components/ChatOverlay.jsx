...
export default function ChatOverlay({
  ...
  const suggestionChips = [
    ...
    {
      label: "🚇 Shortest wait gate right now?",
      query:
        "Which gate has the shortest wait time right now given live turnstile telemetry?",
    },
    {
      label: "🚗 Fastest route home?",
      query:
        "What is the fastest route home right now considering live traffic and transit delays?",
    },
    {
      label: "📢 Re-route crowd tip",
      query:
        "What is your top operational recommendation for relieving the Gate 2 bottleneck?",
    },
  ];
  ...
  const handleSend = async (textToSend = input) => {
    const query =
      typeof textToSend === "string" ? textToSend.trim() : input.trim();
    if (!query) return;
    ...
    try {
      const responseObj = await generateAIResponseAsync(
        query,
        selectedVenueId,
        newMessages,
      );
      ...
      setMessages((prev) => [
        ...prev,
        {
          sender: "ai",
          text:
            `⚠️ **System Error:** Could not generate response (` +
            err.message +
            `). Please verify your network or API settings.`,
          isGroq: false,
        },
      ]);
    } finally {
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
          ...
          <div>
            ...
            <div style={{ fontSize: "0.7rem", opacity: 0.85 }}>
              Multilingual • Groq Llama-3.3 Telemetry
            </div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
          ...
          >
            ...
            {msg.sender === "ai" && msg.latency !== undefined && (
              ...
              >
                <Zap
                  size={11}
                  style={{ color: msg.isGroq ? "#2563eb" : "#f59e0b" }}
                />
                <span>
                  {msg.isGroq
                    ? `Groq Llama-3.3 • ${msg.latency}ms`
                    : `Local Telemetry Engine • ${msg.latency}ms`}
                </span>
              </div>
            )}
          </div>
        ...
        {isTyping && (
          ...
          >
            ...
            <span style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>
              StadiaIQ querying real-time telemetry context...
            </span>
          </div>
        )}
