...
export default function Header({ activeSection, onToggleSidebar, onToggleChat, onOpenSetupModal, selectedVenueId = 'metlife', selectedMatchId = null }) {
  ...
  const [userPrefs, setUserPrefs] = useState({
    language: 'en', isStaff: false, section: '114', row: '12'
  });
  ...
  const sectionLabels = {
    home: 'Home', matches: 'Matches & Venues', route: 'Route & Traffic', navigation: 'Stadium Map',
    crowd: 'Crowd Heatmap', transport: 'Transport Hub', accessibility: 'Accessibility Concierge',
    sustainability: 'Sustainability Tracker', operations: 'Operational Intelligence',
  };

  return (
    <header className="main__header">
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        <button onClick={onToggleSidebar} className="mobile-toggle" aria-label="Toggle Navigation"
          style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0.45rem',
            borderRadius: '6px', background: 'var(--color-gray-100)', color: 'var(--text-primary)', cursor: 'pointer' }}>
          <Menu size={20} />
        </button>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', flexWrap: 'wrap' }}>
        ...
        {activeMatch && (
          <div style={{
            ...
            background: activeMatch.status === 'live' ? 'rgba(239, 68, 68, 0.08)' : 'rgba(37, 99, 235, 0.06)',
            border: `1px solid ${activeMatch.status === 'live' ? '#fca5a5' : 'var(--color-gray-200)'}`,
            borderRadius: '8px', fontSize: '0.72rem', fontWeight: 800, color: 'var(--text-primary)'
          }}>
            {activeMatch.status === 'live' && <span className="live-dot" style={{ width: 6, height: 6 }}></span>}
            <span>{activeMatch.teamA.split(' ').pop()} vs {activeMatch.teamB.split(' ').pop()}</span>
            <span className={`badge badge--${matchStatus.color}`} style={{ fontSize: '0.6rem', padding: '1px 5px' }}>
              {matchStatus.label}
            </span>
          </div>
        )}
        ...
        <div style={{
          ...
          background: 'var(--color-gray-100)', borderRadius: '9999px', fontSize: '0.72rem',
        ...
        <div style={{
          display: 'flex', alignItems: 'center', gap: '0.35rem', padding: '0.3rem 0.6rem',
          background: 'var(--color-success-bg)', borderRadius: '9999px', fontSize: '0.72rem',
        ...
        <button onClick={onOpenSetupModal} title="Change match, seat, or language"
          ...
          {userPrefs.isStaff ? <UserCheck size={13} /> : <Ticket size={13} style={{ color: '#f59e0b' }} />}
          <span>{flags[userPrefs.language] || '🇺🇸'} {userPrefs.isStaff ? 'Staff' : `Sec ${userPrefs.section}`}</span>
        </button>
