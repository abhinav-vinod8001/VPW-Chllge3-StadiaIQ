...
export default function MatchesSection({ onNavigate, onSelectVenue }) {
  ...
  filtered.sort((a, b) => (order[a.status] ?? 3) - (order[b.status] ?? 3) || new Date(a.date) - new Date(b.date));

  return (
    <div className="section active">
      <div className="section__header">
        <h2 className="section__title"><Trophy style={{ color: 'var(--color-gold)' }} /> FIFA World Cup 2026™ Schedule & Venues</h2>
        <p className="section__description">
          Browse all knockout matches across the 16 host venues in the United States, Mexico, and Canada. Select a match to view directions, real-time traffic, or interactive seating maps.
        </p>
      </div>
          ...
          {filtered.map((match) => {
            ...
            return (
              <div 
                ...
                style={{
                  borderLeft: isLive ? '5px solid var(--color-danger)' : isCompleted ? '5px solid var(--color-success)' : '1px solid var(--color-gray-200)'
                }}
              >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
                    <div style={{ flex: 1, minWidth: '260px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.4rem' }}>
                        <span className={`badge badge--${isLive ? 'danger' : isCompleted ? 'success' : 'info'}`}>
                          {isLive && <span className="badge__dot"></span>}
                          {match.round}
                        </span>
                        <span className="text-xs text-muted">{formatDate(match.date)}</span>
                      </div>
                      ...
                      {match.note && <div className="text-xs text-muted" style={{ marginTop: '0.25rem' }}>📝 {match.note}</div>}
                    </div>
                    ...
                    <div style={{ textAlign: 'right', minWidth: '200px' }}>
                      ...
                      <div className="text-xs text-secondary">{venue ? venue.city : ''}</div>
                      {venue && (
