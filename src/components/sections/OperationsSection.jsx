...
export default function OperationsSection({ selectedVenueId = "metlife" }) {
  ...
  const [paTarget, setPaTarget] = useState(
    "Gate 2 East Concourse (16m wait bottleneck)",
  );
  ...
  const handleGeneratePA = async () => {
    ...
    try {
      const res = await generatePABroadcastAsync(
        { type: paTarget, location: paTarget },
        paLang,
        selectedVenueId,
      );
      setPaResult(res);
      ...
      setPaResult({
        script:
          `📢 **PA SCRIPT (` +
          paLang.toUpperCase() +
          `)**\n\nAttention fans: Regarding ` +
          paTarget +
          `, please follow steward instructions and use Gate 4 or Gate 1 for express entry.`,
        isGroq: false,
        latency: 15,
      });
    } finally {
  ...
  const aiRecommendations = [
    {
      title: "Re-route Ingress from Gate 2 to Gate 4",
      impact: `Reduces current ${telemetry.gateWaitTimes.gate2}m wait time down to ~5m at East Concourse.`,
      action: "Auto-Update Digital LED Wayfinding Signs",
    },
    {
      title: "Deploy 6 Extra Stewards to Sector 201 Escalators",
      impact: "Pre-empts crowd density bottleneck forecasted at halftime.",
      action: "Dispatch via Steward Comms Hub",
    },
    {
      title: "Increase Express Train Egress Frequency by +20%",
      impact: `Accommodates early departure surge (current Metro countdown: ${Math.round(telemetry.transitCountdowns.metroGreen / 60)}m).`,
      action: "Notify Transit Operations Control",
    },
  ];
    ...
    <div className="section active">
      <div className="section__header">
        <h2 className="section__title">
          ...
          Operational Intelligence & Command Dashboard
        </h2>
        <p className="section__description">
          Real-time telemetry and predictive decision support for venue staff,
          organizers, and security command. Monitor active incident logs, gate
          throughput, and generate live Groq AI Public Address emergency
          broadcasts.
        </p>
      </div>
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card">
          ...
          <div className="stat-card__value">
            {telemetry.gateWaitTimes.gate2} min
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
            ...
            <div>
              <div style={{ fontWeight: 800, fontSize: "1.05rem" }}>
                Groq AI Public Address (PA) & Broadcast Generator
              </div>
              <div style={{ fontSize: "0.72rem", opacity: 0.85 }}>
                Instantly generate concise, multi-language concourse & LED
                display broadcasts via LPU™ inference
              </div>
            </div>
          </div>
          <span
        ...
        >
          ...
          >
            <div style={{ flex: 2, minWidth: "260px" }}>
              ...
              >
                SELECT ACTIVE BOTTLENECK OR INCIDENT TO BROADCAST
              </label>
              <select
                className="select"
                value={paTarget}
                onChange={(e) => setPaTarget(e.target.value)}
                style={{ fontWeight: 600 }}
              >
                <option
                  value={`Gate 2 East Concourse (${telemetry.gateWaitTimes.gate2}m wait bottleneck)`}
                >
                  🚨 Bottleneck: Gate 2 East Concourse (
                  {telemetry.gateWaitTimes.gate2}m wait) — Re-route to Gate 4
                </option>
                {incidents
                  ...
                  .map((inc) => (
                    <option
                      key={inc.id}
                      value={`${inc.id}: ${inc.type} (${inc.location})`}
                    >
            ...
            <div style={{ flex: 1, minWidth: "180px" }}>
              ...
              <select
                className="select"
                value={paLang}
                onChange={(e) => setPaLang(e.target.value)}
                style={{ fontWeight: 600 }}
              >
            ...
            >
              ...
              >
                ...
                >
                  <span className="badge badge--success">
                    ⚡{" "}
                    {paResult.isGroq
                      ? `Groq Llama-3.3 • ${paResult.latency}ms`
                      : `Local Telemetry Engine`}
                  </span>
                  <span className="text-xs font-bold text-secondary">
                    READY FOR CONCOURSE SPEAKERS & LED DISPLAY BOARDS
                  </span>
                </div>
              ...
              {paBroadcasted && (
                <div
                  ...
                  style={{ marginTop: "0.85rem", padding: "0.6rem 1rem" }}
                >
                  <span className="alert__icon">🔊</span>
                  <div className="alert__content font-bold">
                    [LIVE BROADCAST DISPATCHED]: Audio streamed to 142 concourse
                    speakers and digital LED ribbons across{" "}
                    {selectedVenueId.toUpperCase()}!
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="card">
          <div className="card__header">
            <span className="card__title">
              <Cpu size={18} style={{ color: "var(--color-primary-light)" }} />{" "}
              ...
              >
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() =>
                      alert(
                        `StadiaIQ Command: Executed "${rec.action}" successfully across venue telemetry network.`,
                      )
                    }
                  >
                    Execute Command ⚡
          ...
          >
            ...
            >
              <span className="text-xs font-semibold text-primary">
                💡 StadiaIQ Telemetry Tip: Turnstile ingress rate at Gate 1 is
                running at 115 fans/min. No steward redeployment needed.
              </span>
            </div>
          </div>
        </div>
              ...
              <tbody>
                {incidents.map((inc) => (
                  <tr key={inc.id}>
                    ...
                    <td>
                      <span
                        className={`badge badge--${inc.priority === "High" ? "danger" : inc.priority === "Medium" ? "warning" : "info"}`}
                      >
                        {inc.priority}
                      </span>
                    </td>
