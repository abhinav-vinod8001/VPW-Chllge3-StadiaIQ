...
export default function AccessibilitySection() {
  ...
  const features = [
    {
      icon: '♿', title: 'Step-Free Wheelchair Access',
      items: [
    ...
    {
      icon: '👁️', title: 'Visual & Audio Assistance',
      items: [
    ...
    {
      icon: '🧠', title: 'Sensory Rooms & Quiet Zones',
      items: [
    ...
    {
      icon: '👂', title: 'Hearing & Deaf Assistance',
      items: [
  ...
  return (
    <div className="section active">
      <div className="section__header">
        <h2 className="section__title"><Accessibility style={{ color: 'var(--color-primary-light)' }} /> Accessibility Concierge & Step-Free Routing</h2>
        <p className="section__description">
          Comprehensive accessibility guides for fans of all abilities. Plan 100% step-free routes between entry gates and seating tiers, or locate sensory rooms and assistance desks.
        </p>
      </div>
          ...
          <div key={idx} className="card hover-lift">
            <div className="card__header">
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                ...
                <span className="card__title" style={{ fontSize: '1.1rem' }}>{f.title}</span>
              </div>
            </div>
              ...
              <ul style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
                {f.items.map((item, i) => (
                  <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem', fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                    <Check size={16} style={{ color: 'var(--color-success)', flexShrink: 0, marginTop: '2px' }} />
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
      ...
      <div className="card" style={{ marginTop: 'var(--space-6)' }}>
        <div className="card__header">
          <span className="card__title"><Navigation size={18} style={{ color: 'var(--color-primary-light)' }} /> AI Accessible Route Planner</span>
          <span className="badge badge--info">100% Step-Free Path Verified</span>
        </div>
        <div className="card__body">
          <form onSubmit={handlePlanRoute} style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'flex-end' }}>
            <div style={{ flex: 1, minWidth: '220px' }}>
              <label className="text-xs text-muted font-bold" style={{ display: 'block', marginBottom: '0.4rem' }}>STARTING POINT</label>
            ...
            <div style={{ flex: 1, minWidth: '220px' }}>
              <label className="text-xs text-muted font-bold" style={{ display: 'block', marginBottom: '0.4rem' }}>DESTINATION</label>
          ...
          {plannedRoute && (
            <div className="alert alert--success" style={{ marginTop: '1.5rem' }}>
              ...
              <div className="alert__content">
                <div className="alert__title">Step-Free Route Generated: {plannedRoute.from} → {plannedRoute.to}</div>
                <div style={{ fontSize: '0.85rem', marginTop: '0.5rem' }}>
                  <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '0.75rem', fontWeight: 700 }}>
                    <span>📏 Distance: {plannedRoute.distance}</span>
                    <span>⏱️ Est. Time: {plannedRoute.time}</span>
                  </div>
                  <ol style={{ paddingLeft: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.4rem', listStyle: 'decimal' }}>
