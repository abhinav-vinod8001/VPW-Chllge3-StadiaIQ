...
export default function HomeSection({
  ...
  const activeMatchStatus = activeMatch
    ? getMatchStatusDisplay(activeMatch)
    : null;
    ...
    <div className="section active">
        ...
        >
          <div>
            ...
            <p className="section__description">
              Your real-time AI operational companion for the FIFA World Cup
              2026™. Everything below is auto-configured for your selected match
              and venue.
            </p>
          </div>
          <div
          ...
          style={{
            ...
            borderLeft:
              activeMatch.status === "live"
                ? "5px solid #ef4444"
                : "5px solid var(--color-primary-light)",
            background:
              activeMatch.status === "live"
      ...
      <div className="stats-grid">
        <div className="stat-card hover-lift">
          <div className="stat-card__header">
            ...
            <span className="stat-card__trend stat-card__trend--up">
              ↑ {telemetry.capacityPercentage}% Cap
            </span>
          </div>
        ...
        <div className="stat-card hover-lift">
          <div className="stat-card__header">
            ...
            >
              Gate 4: {telemetry.gateWaitTimes.gate4}m
            </span>
          </div>
          <div className="stat-card__value">
            {Math.round(
              (telemetry.gateWaitTimes.gate1 +
                telemetry.gateWaitTimes.gate3 +
                telemetry.gateWaitTimes.gate4) /
                3,
            )}{" "}
            min
          </div>
      ...
      <div className="card" style={{ marginTop: "var(--space-2)" }}>
        ...
        <div className="card__body">
          <div className="action-chips">
            ...
            >
              <Trophy size={14} /> All Matches
            </button>
            <button className="action-chip" onClick={() => onNavigate("route")}>
              <MapPin size={14} /> Directions & Traffic
            </button>
            <button
              className="action-chip"
              onClick={() => onNavigate("navigation")}
            >
              <Compass size={14} /> Find My Seat
            </button>
            <button className="action-chip" onClick={() => onNavigate("crowd")}>
              <Users size={14} /> Crowd Status
            </button>
            <button
              className="action-chip"
              onClick={() => onNavigate("transport")}
            >
              <Train size={14} /> Journey Home
            </button>
            <button
              className="action-chip"
              onClick={() => onNavigate("accessibility")}
            >
              <Accessibility size={14} /> Accessible Routes
            </button>
            <button
              className="action-chip"
            ...
            <button className="action-chip" onClick={onOpenChat}>
      ...
      <div className="content-grid" style={{ marginTop: "var(--space-6)" }}>
        <div className="card">
          <div className="card__header">
            <span className="card__title">
              <Bot size={18} style={{ color: "var(--color-primary-light)" }} />{" "}
            ...
            >
                ...
                <div key={i} className={`alert alert--${alt.color}`}>
                  ...
                  <div className="alert__content">
                    <div className="alert__title">
                      {alt.road} — {alt.status.toUpperCase()} (+{alt.delay}m)
                    </div>
                    Real-time DOT sensor alert at {alt.time}. Take alternate
                    approach routes or public rail express.
                  </div>
                </div>
              ))}
              <div className="alert alert--info">
                ...
                <div className="alert__content">
                  <div className="alert__title">
                    Next Express:{" "}
                    {Math.round(telemetry.transitCountdowns.metroGreen / 60)}m{" "}
                    {telemetry.transitCountdowns.metroGreen % 60}s
                  </div>
                  Leave via Gate 4 (West Stand) 5 minutes early for zero-wait
                  rail boarding.
                </div>
              </div>
              <div className="alert alert--success">
                ...
                <div className="alert__content">
                  ...
                  Attendance crossed {telemetry.attendance.toLocaleString()}{" "}
                  with 68% waste diversion rate!
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="card">
          <div className="card__header">
            <span className="card__title">
              <Trophy size={18} style={{ color: "#f59e0b" }} /> Upcoming Matches
            </span>
            <button
                ...
                <div key={match.id} className="list-item">
                  ...
                  <div className="list-item__content">
                    ...
                    <div className="list-item__subtitle">
                      {match.round} • {formatDate(match.date)} • {match.time} ET
                    </div>
                    <div
                      className="list-item__subtitle"
                      style={{ opacity: 0.85 }}
                    >
                      🏟️ {venue ? `${venue.name}, ${venue.city}` : ""}
                    </div>
                  </div>
                  <button
                    className="btn btn--sm btn--outline"
