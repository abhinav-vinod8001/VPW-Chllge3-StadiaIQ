...
export default function GroqConfigModal({ isOpen, onClose }) {
  ...
  useEffect(() => {
    ...
    const storedModel =
      localStorage.getItem("stadiaiq_groq_model") || "llama-3.3-70b-versatile";
    setApiKey(storedKey);
  ...
  const handleTestInference = async () => {
    ...
    if (!keyToUse) {
      setTestResult({
        success: false,
        error: "Please enter a valid Groq API key first.",
      });
      return;
    }
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are StadiaIQ, the real-time AI operational assistant for the FIFA World Cup 2026.",
              },
              {
                role: "user",
                content:
                  "Test connection: State your operational readiness in one short sentence.",
              },
            ],
            max_tokens: 60,
            temperature: 0.3,
  ...
  const previewPayload = {
    ...
    system_prompt_injection: {
      venue: liveWeatherSample
        ? `${liveWeatherSample.venueName}, ${liveWeatherSample.city}`
        : "MetLife Stadium, NJ",
      live_weather: liveWeatherSample
        ? `${liveWeatherSample.temp}°C, ${liveWeatherSample.condition}, Wind ${liveWeatherSample.windSpeed} km/h`
        : "24°C, Clear",
      ...
      open_incidents: telemetry.incidents
        .filter((i) => i.status !== "Resolved")
        .map((i) => `${i.id}: ${i.type} (${i.location})`),
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
            ...
            <div>
              <div style={{ fontWeight: 800, fontSize: "1.15rem" }}>
                Groq AI Command Center & Real-Time Engine
              </div>
              <div style={{ fontSize: "0.75rem", opacity: 0.85 }}>
                Ultra-Low Latency LLM Inference & Live Context Injection
              </div>
            </div>
          </div>
          <button
            onClick={onClose}
            style={{ color: "white", opacity: 0.85, cursor: "pointer" }}
          >
            <X size={22} />
          </button>
        </div>
        ...
        >
          ...
          >
            ...
            <div className="alert__content">
              <div className="alert__title">
                Real-World Live Telemetry Injection
              </div>
              When configured with your Groq API key, StadiaIQ sends live venue
              conditions (weather, attendance, turnstile delays, active road
              incidents) to Groq's high-speed LPU™ inference engine (`300+
              tokens/sec`), providing instant, real-world actionable wayfinding
              and emergency PA broadcasts.
            </div>
          </div>
          ...
          <div>
            ...
            >
              ...
              and never transmitted to any external server other than directly
              to `api.groq.com`.
            </div>
          </div>
          ...
          <div>
            ...
            >
              <option value="llama-3.3-70b-versatile">
                llama-3.3-70b-versatile (Recommended — Deep Reasoning &
                Multi-Language PA Broadcasts)
              </option>
              <option value="llama-3.1-8b-instant">
                llama-3.1-8b-instant (Ultra-Fast — Sub-150ms Wayfinding &
                Navigation)
              </option>
          ...
          {saved && (
            ...
            >
              ...
              <div className="alert__content font-semibold">
                Settings saved to localStorage (`stadiaiq_groq_key`). Live Groq
                inference enabled!
              </div>
            </div>
          )}
          ...
          {testResult && (
            <div
              className={`alert alert--${testResult.success ? "success" : "danger"}`}
            >
              <span className="alert__icon">
                {testResult.success ? (
                  <CheckCircle size={20} />
              ...
              <div className="alert__content">
                <div className="alert__title">
                  {testResult.success
                    ? `✅ Groq Inference Success — ${testResult.latency}ms Latency!`
                    : "❌ Connection Test Failed"}
                </div>
                {testResult.success ? (
                  <div style={{ fontSize: "0.85rem", marginTop: "0.4rem" }}>
                    <div>
                      <strong>Model Reply:</strong> "{testResult.reply}"
                    </div>
                    <div
                      style={{
                        display: "flex",
                    ...
                    >
                      <span>
                        ⚡ Latency: <strong>{testResult.latency} ms</strong>
                      </span>
                      <span>
                        🔢 Tokens: <strong>{testResult.tokens} total</strong>
                      </span>
                    </div>
                  </div>
                ) : (
          ...
          >
            ...
            >
              <span className="text-xs font-bold text-muted">
                🔴 REAL-TIME INJECTED SYSTEM PROMPT PAYLOAD (PREVIEW)
              </span>
              <span
